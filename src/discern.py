"""Discern class for discerning the best black letter law provision.

## Attributes

- `extract`: An instance of the `Extract` class, which contains the groups of rules from which
the consensus, disagreements, and unique rules will be extracted.
...
- `explanation`: Explanation of the rule, which is passed to the `Comment` class as notes for 
determining what comments to write.
...
- `__init__(self, extract, section)`: Initializes the `Discern` class with an instance of the 
`Extract` class and a section.
- `get_consensus(self)`: Discerns the consensus rule based on description of consensus produced by 
`Extract` class.
- `get_disagreement(self)`: Decides what points of disagreement to include in the final rule based 
on notes about disagreement produced by `Group` class instance.
- `resolve(self)`: Resolves each point of disagreement by creating instances of `Resolve` class for
each point of disagreement.
- `get_resolve_rule(self)`: Creates a rewritten rule following the resolve process. It uses the 
LLMChain to generate the rewritten rule and saves it to `self.resolve_rule`.
- `get_clear(self)`: Creates notes on how the rule could be made more clear and logical. It uses 
the LLMChain to generate the notes and saves them to `self.clear`.
- `get_edit(self)`: Creates a revised, final rule based on notes on how the rule could be made more
clear and logical. It uses the LLMChain to generate the final rule and saves it to `self.final`.
- `set_explanation(self)`: Sets the explanation of the rule, which is a string passed to the 
`Comment` class as notes for determining what comments to write.
...
class Discern(BaseClass):
    ...
    Resolve class to resolve each point of disagreement, combines the consensus rule with the 
    resolved rules, creates notes on how the rule could be made more clear and logical, and creates 
    ...
    def get_disagreement(self):
        ...
        logger.info(
            "get_disagreement: Getting points of disagreement to address in final rule.")
        output, total_tokens, model, prompt_lst = llm_router(
            prompt_system,
            prompt_human,
            query,
        ...
        self.disagreement_lst = [
            item for item in self.disagreement_lst if len(item) >= 100]

        # Save the prompts used in this method
        self.prompt_lst.append(save_used_prompts(
            "## Disagreement prompts", prompt_lst))
    ...
    def resolve(self):
        ...
        logger.info("Number of points of disagreement: %s",
                    len(self.disagreement_lst))
        issue_tally = 1
        self.resolve_outputs = ""
    ...
    def get_resolve_rule(self):
        ...
        logger.info(
            "get_resolve_rule: Getting rewritten rule following the resolve process.")
        output, total_tokens, model, prompt_lst = llm_router(
            prompt_system,
            prompt_human,
            query,
    ...
    def get_clear(self):
        ...
        logger.info(
            "get_clear: Getting notes on how the rule could be made more clear and logical.")
        output, total_tokens, model, prompt_lst = llm_router(
            prompt_system,
            prompt_human,
            query,
        ...
        # Save the prompts used in this method
        self.prompt_lst.append(save_used_prompts(
            "## Clear prompts", prompt_lst))

        # Sleep for tokens
        sleep_for_tokens(total_tokens, model)

    def get_edit(self):
        """Create a revised, final rule based on notes for making rule more clear and logical.
        """
        # Set prompts for LLM.
        # Set system prompt with contents from txt file
        prompt_system = set_full_prompt(
            os.path.join(get_root_dir(), "data", "prompts",
                         'discern', "prompt_edit.txt"),
            self.section
        )
    ...
    def save_to_md(self):
        ...
        self.save_prompts_to_md("discern_prompts")
        ...
        with open(
            ...
            f.write("# Clear\n")
            f.write(
                "*Notes on how the rule could be made more clear and logical.* \n\n")
            f.write(self.clear)
            f.write("\n\n")
